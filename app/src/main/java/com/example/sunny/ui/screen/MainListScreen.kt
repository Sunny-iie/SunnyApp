...
//) {
//    // 1. 定义分类状态（null 表示显示“全部”）
//    var selectedCategory by remember { mutableStateOf<String?>(null) }
//    val categories = listOf("食物", "药品", "护肤品", "化妆品")
//
//    val currentTime = System.currentTimeMillis()
//
//    // 2. 核心过滤逻辑：先过滤分类，再根据时间拆分
...
//                .horizontalScroll(rememberScrollState()) // 支持横向滑动
...
//                activeColor = MorandiDark // 全部按钮使用深色
//            )
//
//            // 各个分类按钮
...
//                        Text("⚠️ 需处理", modifier = Modifier.padding(start = 20.dp, bottom = 8.dp),
//                            style = MaterialTheme.typography.labelMedium, color = MorandiRed.copy(alpha = 0.7f))
//                    }
//                    items(expiredItems, key = { "exp_${it.id}" }) { item ->
//                        SwipeToDeleteWrapper(
//                            item = item,
...
//                        Text("✅ 存储中", modifier = Modifier.padding(start = 20.dp, top = 12.dp, bottom = 8.dp),
//                            style = MaterialTheme.typography.labelMedium, color = MorandiGreen.copy(alpha = 0.7f))
//                    }
//                    items(activeItems, key = { "active_${it.id}" }) { item ->
//                        SwipeToDeleteWrapper(
//                            item = item,
...
//        border = BorderStroke(
//            width = 1.dp,
//            color = if (isSelected) activeColor.copy(alpha = 0.5f) else MorandiBlue.copy(alpha = 0.2f)
//        )
//    ) {
//        Text(
//            text = label,
...
//            color = if (isSelected) activeColor else MorandiDark.copy(alpha = 0.6f),
